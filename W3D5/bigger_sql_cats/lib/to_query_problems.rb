...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    WHERE 
      shiny.name = 'Shiny Mouse' AND not_shiny.name != 'Shiny Mouse'
    ORDER BY
      not_shiny.name;
  SQL
end
...
def toys_that_jet_owns_sub
  ...
  # USE A SUBQUERY
  execute(<<-SQL)
  select distinct cats.name, toys.name from cats
  join cattoys on cat_id = cats.id
  join toys on toy_id = toys.id
  where toys.name IN (select toys.name from toys
  join cattoys on toys.id = toy_id
  join cats on cat_id = cats.id
  where cats.name = 'Jet') AND cats.name != 'Jet'
  order by cats.name
